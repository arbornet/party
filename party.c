...
main(int argc, char **argv)
{
	...
	if (finduser(logtty, logname, &logtime))
		exit(1);
	...
	if (opt[OPT_ARG].yes)
		for (n = 1; n < argc; n++) parseopts(argv[n], 2);
	...
	if (join_party(opt[OPT_START].str))
		exit(1);
		...
		if (output()) {
			...
			if (mailfuse-- == 0) {
				...
				if (opt[OPT_IDLEOUT].yes &&
					...
					fprintf(stderr,
					    "You have been idle in party for "
					    "more than %s minutes\nBye!\n",
					    opt[OPT_IDLEOUT].str);
					done(0);
				}
...
void
help(char *filename, int complain)
{
	register int hf;
	register int n;
...
char *
chn_file_name(char *chn, int keeplog)
{
	char *file;
	size_t len;
	...
	snprintf(file, len, "%s/%s.%s", opt[OPT_DIR].str, chn,
	    keeplog ? "log" : "tmp");
	return (file);
}
 ...
 */
char *
chn_lockfile_name(char *chn, int keeplog)
{
	char *lockfile;
	size_t len;

	len = strlen(opt[OPT_DIR].str) + strlen(chn) + 1 + strlen(".lock") + 1 +
	      strlen("log") + 2 + 1;
	...
	snprintf(lockfile, len, "%s/.lock/%s.%s", opt[OPT_DIR].str, chn,
	    keeplog ? "log" : "tmp");
	return (lockfile);
}
...
join_party(char *nch)
{
	...
	if (nch != NULL) {
		...
		if ((!opt[OPT_MAYCLOSE].yes && access(file, 4)) ||
			...
			if (!access(file, 0) ||
			    (tmp_rst = open(file, O_RDONLY | O_CREAT,
			         opt[OPT_MAYCLOSE].yes ? DEP_MODE : CHN_MODE)) <
			        0) {
		...
		if (debug)
			db("join_party: new channel open\n");
	}
	...
	if (channel != NULL) {
		...
		if (debug)
			db("join_party: old channel closed\n");
	}
