...
initoutput()
{
	...
	ob[BUF_FILE].n = 0;
	for (i = 1; i < MAXFIL; i++) ob[i].bf = NULL;
}
...
bufwrite(int in)
{
	...
	{
	case BUF_EATLINE:
		if (!opt[OPT_RAW].yes || !opt[OPT_SHOWNOISE].yes ||
		    ignoring != NULL || !opt[OPT_SHOWREAD].yes ||
		    !opt[OPT_SHOWEVENT].yes || !opt[OPT_REPEAT].yes) {
			bufopen(BUF_EATLINE);
			eatline(in);
	...
	default:
		if (write(out_fd, ob[in].bf, ob[in].n) < 0 && errno == EPIPE &&
		    out_fd != 1) {
...
bgets(int in)
{
	...
	if (ob[in].i > 0) {
		...
		strncpy(
		    ob[in].bf, ob[in].bf + ob[in].i, (k = ob[in].n - ob[in].i));
		ob[in].n = k;
		ob[in].i = 0;
		return NULL;
	}
...
eatline(int in)
{
	...
	while ((line = bgets(in)) != NULL)
		if ((opt[OPT_SHOWNOISE].yes || line[0] != '<') &&
			...
			bputc(out, '\n');
			if (!opt[OPT_REPEAT].yes)
				linecpy(oldline, line);
		}
}
