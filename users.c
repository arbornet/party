...
void
add_ulist(struct ulist **, char *, char *ch, time_t, char *, char);

void
wscan_init(void);
int
wscan_next(struct pwho *, struct sub_pwho *, int *);
int
wscan_done(void);
...
who_list(FILE *fp, char sortkey)
{
	...
	while (wscan_next(&ahead, &abody, &n))
		add_ulist(&head, abody.alias, abody.channel, abody.time,
		    abody.shelled, sortkey);
	wscan_done();
	...
	for (curr = head; curr != NULL; curr = curr->next) {
		fprintf(fp, "%-*.*s %15.15s  %.*s", UT_NAMESIZE, UT_NAMESIZE,
		    curr->alias, ctime(&curr->time) + 4, CHN_LEN,
		    curr->channel);
		if (curr->shelled[0] == '\0')
			fprintf(fp, "\n");
#ifdef SIGTSTP
		else if (curr->shelled[0] == '^' && curr->shelled[1] == '\0')
...
void
add_ulist(
    struct ulist **head, char *al, char *ch, time_t tm, char *sh, char sortkey)
{
	...
	for (curr = *head, prev = NULL; curr != NULL;
		...
		if ((sortkey == 'c' &&
		    ...
		    (sortkey == 'n' && (c_al < 0 || (c_al == 0 && c_tm < 0))) ||
		    (sortkey == 't' && c_tm < 0))
			break;
	}
...
int
who_uniqalias(char *alias, char *name, char *channel)
{
	struct pwho ahead;
	struct sub_pwho abody;
	int n, skipping = 0;
...
int
wscan_next(struct pwho *phead, struct sub_pwho *pbody, int *n)
{
	struct passwd *pw;
	struct stat s;
...
void
ncstrncpy(char *s1, char *s2, int n)
{
...
int
finduser(char *logtty, char *logname, time_t *logtime)
{
	struct passwd *pw;
	const char *tty;
	int i;
