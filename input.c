...
#include <sys/types.h>     // for lseek, time_t
...
char realname[UT_NAMESIZE + 2] =
    "";                        /* User's real name (used for mail check) */
...
struct cc_tab {
    ...
    {'/', '/', cmd_noise, OPT_MAKENOISE, "No noises allowed in this channel\n"},
...
#define BS 1024
	...
	*firstin(filename, "\n") = '\0';
	...
	fprintf(stderr, "%s to file %s...",
	    ftell(fp) == 0L ? "Saving" : "Appending", filename);
	fflush(stderr);
...
setname(char *chan)
{
	...
	if (opt[OPT_UIDNAME].yes)
		strcpy(
		    name, opt[OPT_RENAME].yes ? opt[OPT_ALIAS].str : realname);
	else {
	...
	if (!havename) {
		if (opt[OPT_RANDNAME].yes && opt[OPT_RANDNAME].str[0] != '\0') {
			...
			if ((fp = fopen(opt[OPT_RANDNAME].str, "r")) == NULL)
				...
				/* Initialize random number generator, adding
				   some garbage to the second so two users
				   entering at the same time don't get the same
				   name */
			...
			for (;;) {
				fprintf(stderr,
				    "What name would you like (8 chars max)? ");
				...
				if (txbuf[0] == '\n') {
					if (opt[OPT_RENAME].yes) {
						 ...
						 */
						strcpy(
						    name, opt[OPT_ALIAS].str);
						checkname(name);
						if (who_uniqalias(
						        name, logname, chan))
							break;
					}
					/* He can always use his real name,
					 * unique or not */
...
stashname()
{
	...
	strcpy(inbuf, name);
	inbuf[namelen] = ':';
	for (i = namelen + 1; i < headlen; i++) inbuf[i] = ' ';
}
...
void
append(char *string, FILE *wfd, int lfd)
{
	LOCK(lfd);
	fseek(wfd, 0L, 2);
	fputs(string, wfd);
...
initmodes()
{
	...
	errno = 0;
	if (debug)
		db("reading stty modes\n");
	GTTY(0, &cooked);
#ifdef DEBUG_STTY
	if (debug) {
		db("cooked modes read\n");
...
char *
pgetline(char *bf, int n, int ch1)
{
	int col, ch, i;
	char *rc;

	if (isascii(ch1) && isprint(ch1) && opt[OPT_FIRSTCHAR].yes) {
		/* Read in cbreak mode, doing as much of our own cooking as
		 * possible */
		putchar(ch1);
		bf[0] = ch1;
		col = 1;
		for (;;) {
			ch = getchar();
			if (ch == '\t')
				ch = ' '; /* Tabs are a pain--get rid of them */
			if (ch == '\n') {
				bf[col++] = ch;
				bf[col] = '\0';
				putchar(ch);
